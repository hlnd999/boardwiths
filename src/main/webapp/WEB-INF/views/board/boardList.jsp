	...
	<div class="container d-flex m-auto" style="width: 1440px;">
	    ...
	    <!-- 게시판 top -->
	        ...
	        <c:if test="${ loginUser ne null }">
		        <c:choose>
					<c:when test="${ selCategory eq '1' and checkMgr eq 'Y'}">
						<div class="d-block text-end">
		        			<button class="btn btn-primary" type="button" onclick="location.href='${ contextPath }/notice.bo'">글쓰기</button>
		   				</div>
					</c:when>
					<c:when test="${ selCategory eq '2,3' or selCategory eq '2' or selCategory eq '3'}">
						<div class="d-block text-end">
		        			<button class="btn btn-primary" type="button" onclick="location.href='${ contextPath }/buy.bo'">글쓰기</button>
		   				</div>		
					</c:when>
					
					<c:when test="${ selCategory eq '4,5' or selCategory eq '4' }">
						<div class="d-block text-end">
		        			<button class="btn btn-primary" type="button" onclick="location.href='${ contextPath }/matching.bo'">글쓰기</button>
		   				</div>
					</c:when>
					<c:when test="${ selCategory eq '5' }">
						<div class="d-block text-end">
		        			<button class="btn btn-primary" type="button" onclick="location.href='${ contextPath }/review.bo'">글쓰기</button>
		   				</div>
					</c:when>
					<c:when test="${ selCategory eq '6' }">
						<div class="d-block text-end">
